...
from src.schemas.book import Book, BookAdd, BookAddRequest, BookPatch, BooksAuthorsAdd, BookPatchRequest
...
class BookService(BaseService): 
    async def create_book(self, book_data: BookAddRequest) -> Book:
        ...
        books_authors_data = [
            BooksAuthorsAdd(book_id=new_book.id, author_id=a_ids) for a_ids in set(book_data.author_ids)
        ]
    ...
    async def edit_book(self, id: int, book_data: BookPatchRequest) -> Book:
        ...
        try:
            ...
            await self.db.books_authors.edit_authors_ids(book_id=id, new_authors_ids=book_data.author_ids)
